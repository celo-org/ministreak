 ...
 * Returns a Set of "player:roundId:dayIndex" keys that are already submitted.
 */
export async function checkAlreadySubmitted(
  ...
  for (let i = 0; i < results.length; i++) {
    if (results[i].status === "success" && results[i].result === true) {
      const key = `${qualifyingTxs[i].player.toLowerCase()}:${qualifyingTxs[i].roundId}:${qualifyingTxs[i].dayIndex}`;
